...
- **코드 문체 앵커 = AnimalBreakOut 코드베이스** (유저가 손코딩기에 흡수한 팀 컨벤션 — 2026-09-07 결정).
  구체적으로: 이른 반환(가드 문) 선호, 여러 줄 `&&` 체인 지양, **메서드는 중괄호 블록으로 쓴다**
  (식 본문 `=>`는 러너에도 있던 한 줄 상태 변경·계산 정도만), **람다는 외부 API가 함수를 요구할 때만**
  — 콜백은 이름 붙은 private 메서드로 빼서 넘긴다(`CreateUnit`·`OnGetFromPool`). `out var` 같은 압축도 피한다.
  코스모는 구조·판례 앵커(매니저 관계·주석 문체)로만 쓰고, 문장 스타일은 러너를 따른다
